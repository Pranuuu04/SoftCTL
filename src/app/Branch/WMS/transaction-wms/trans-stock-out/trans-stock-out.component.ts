...
@Component({
  selector: 'app-trans-stock-out',
  templateUrl: './trans-stock-out.component.html',
  styleUrls: ['./trans-stock-out.component.css']
})
export class TransStockOutComponent implements OnInit {
  ...
  this.http.get<any>(`${environment.apiUrl}/Booking/getSerialNo?itemNo=${this.selectedItem}`).subscribe((data: any) => {
   ...
   onAddItem(): void {
     ...
     if (itemGroup.valid) {
       ...
       this.StockOutDetails.push(newItem);
   ...
   removeItem(index: number): void {
     this.StockOutDetails.splice(index, 1);
   }
   ...
   generateStock(formData: any): void {
    ...
    //    }
     const body = {
       ...
       StockOutDetails: this.StockOutDetails
     };
     this.wmsservice.createStockOut(body).subscribe({
       next: (resp: any) => {
         if (resp.status === 1) {
 ...
 getItem() {
  this.wmsservice.getItem(this.selectedSerialNo).subscribe((resp: any) => {
...
onFileSelected(event: any): void {
 ...
 this.StockOutDetails = data.map(row => ({
...
resetImport(fileInput: HTMLInputElement): void {
  fileInput.value = '';        
  this.StockOutDetails = [];
}
 generateExcel() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        FileSaver.saveAs(blobData, 'StockOutTemplate.xlsx');
      } else {
    ...
    processExcelData(file: File) {
     ...
     // const custCode = this.createForm.get('custName')?.value;
