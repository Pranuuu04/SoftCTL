...
export class CategoryComponent implements OnInit {
     ...
     displayedColumns: any[] = [ 'action', 'Category_Code', 'Category_Name'];
     zoneViewData: any[] = [];
     ...
     openCategoryForm(element) {
       const dialogRef = this.dialog.open(CategoryFormComponent, {
         data: {
           action:element ? 'categoryEdit' : 'categoryAdd',
           categoryData: element || {},
           categoryMode:element ? 'edit' : 'add'
         },
         width: '30rem',
         disableClose: true
       });
     ...
     deleteCategory(element): void {
      this.httpService.get(`${environment.apiUrl}Master/allMasters?masterName=Category&operation=deleteCategory&code=${element.Category_Code}&name=`).then((res: any) =>{
        console.log('formdata:', res.status);
        if (res.status == 1) { 
          // this.getCategoryData(); 
          this.openSnackBar(res.message , 'custom-snackbar' );
