...
export class ItemReportComponent implements OnInit {
  ...
  displayedColumns: string[] = ['S_no', 'Category_Code', 'Item_Code', 'Item_Name', 'Qty', 'Rate', 'Volume'];
...
getItem() {
  this.wmsservice.getItem(this.serialNo).subscribe((resp: any) => {
    //  this.itemList = resp.Data;
     this.itemList = resp.Data;
    this.filteredItemList = [{ ItemCode: 'All', ItemName: 'All Item' }, ...this.itemList];
  });
 }
 ...
 openSnackBar(message: string, panelClass: string) {
   ...
   openprogressbar(): MatDialogRef<ProgressBarComponent> {
      ...
      downloadSample() {
        ...
        if (isConfirmed) {
            this.wmsservice.getWMSItemNameReport(
  ...
  handlePageEvent(e: PageEvent) {
    ...
    this.generateItemReport(formData);
  }
  resetPagination() {
    this.pageIndex = 0;
    this.paginator.pageIndex = 0;
  }
  generateItemReport(formData: any) {
    ...
    this.wmsservice.getWMSItemNameReport(
      ...
      error: () => {
        this.openSnackBar('Failed to fetch item reports', 'error-snackbar');
      }
    });
  }
