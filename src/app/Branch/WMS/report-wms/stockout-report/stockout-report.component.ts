...
@Component({
  selector: 'app-stockout-report',
  templateUrl: './stockout-report.component.html',
  styleUrls: ['./stockout-report.component.css']
})
...
getItem() {
  this.wmsservice.getItem(this.serailNo).subscribe((resp: any) => {
 ...
 openSnackBar(message: string, panelClass: string) {
   ...
   openprogressbar(): MatDialogRef<ProgressBarComponent> {
      ...
      downloadSample() {
        ...
        if (isConfirmed) {
            this.wmsservice.getWMSStockOutReport(
              ...
              if (response.status === 1 && Array.isArray(response.Data)) {
                ...
                XLSX.writeFile(wb, 'stockOut_Details.xlsx');
                progressBar.close();
  ...
  StockInReport(formData: any) {
    ...
    this.wmsservice.getWMSStockOutReport(
      ...
      error: () => {
        this.openSnackBar('Failed to fetch Stock out reports', 'error-snackbar');
      }
    });
  }
