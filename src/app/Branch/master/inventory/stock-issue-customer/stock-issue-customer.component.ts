...
@Component({
  selector: 'app-stock-issue-customer',
  templateUrl: './stock-issue-customer.component.html',
  styleUrls: ['./stock-issue-customer.component.css']
})
export class StockIssueCustomerComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngOnChanges(): void {
    if (this.customerData.Data && this.customerData.Data.length > 0) {
      this.dataSource = new MatTableDataSource<any>(this.customerData.Data);
  ...
  openEntryForm(element) {
    const dialogRef = this.dialog.open(InventoryFormComponent, {
      data: {
        action: 'CustAdd',
        stockCustomerData: element,
        stockCustomerMode: 'edit'
      },
      width: '30rem',
      disableClose: true
    });
