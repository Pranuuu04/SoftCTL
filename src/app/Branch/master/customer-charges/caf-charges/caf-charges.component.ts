...
@Component({
  selector: 'app-caf-charges',
  templateUrl: './caf-charges.component.html',
  styleUrls: ['./caf-charges.component.css']
})
export class CafChargesComponent implements OnInit {
  ...
  @ViewChild(MatPaginator) paginator: MatPaginator;
   constructor(private dialog: MatDialog,private custChrgService:CustomerChrgService,private snackBar:MatSnackBar) { }
     ...
     openCafCharForm(element) {
       const dialogRef = this.dialog.open(CustChargFormComponent, {
         data: {
           action: element ? 'CafCharEdit' : 'CafCharAdd',
           CafCharData:  element || {},
           CafCharMode: element ? 'edit' : 'add'
         },
         autoFocus: false,
         width: '45rem',
         disableClose: true
    ...
    //  Club_No: 'ClubNo',
     ...
     getCafChrgData(){
      this.custChrgService.getCafCharge().subscribe((res:any)=>{
       if(res.status === 1){
      ...
      deleteCafChrgData(element){
        this.custChrgService.deleteCafCharge(element.Fuel_Code).subscribe((res:any)=>{
          if(res.status === 1){
            this.openSnackBar(res.message , 'custom-snackbar');
