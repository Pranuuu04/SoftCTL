...
@Component({
  selector: 'app-audit-booking',
  templateUrl: './audit-booking.component.html',
  styleUrls: ['./audit-booking.component.css']
})
export class AuditBookingComponent implements OnInit, AfterViewInit {
  ...
  pageIndex = 0;
  destinationList: any[] = [];
  paginatedDestinationList: any[] = [];
  filteredDestinationList: any[] = [];
  scrollTrigger$: Subject<void> = new Subject<void>();
  destinationName: any;
...
// loadAllCharges() {
//   const charges = Object.keys(this.chargeMap);
//   const customerCode = this.bookingForm.value.customerCode;
//   const productCode = this.bookingForm.value.productCode;
//   const amount = this.bookingForm.value.amount;
//   const bookDate = this.bookingForm.value.bookDate;
//   const destinationCode = this.bookingForm.value.destinationCode;

//   charges.forEach(chargeName => {
//     this.auditService.getChargeValue(chargeName, customerCode, productCode, amount, bookDate, destinationCode)
//       .subscribe(
//         (res: any) => {
//           const value = res?.data ?? 0;
//           const controlName = this.chargeMap[chargeName];
//           if (this.bookingForm.controls[controlName]) {
//             this.bookingForm.controls[controlName].patchValue(value);
//           }
//         },
//         error => {
//           console.error(`Error loading ${chargeName}`, error);
//         }
//       );
//   });
// }
// fetchCharge(apiChargeName: string, formControlKey: string) {
//   const customerCode = this.bookingForm.value.consignerName;
//   const productCode = this.bookingForm.value.productmode;
//   const freightAmt = this.bookingForm.value.freightAmt;
//   const bookDate = this.bookingForm.value.bookDate;
//   const destinationCode = this.bookingForm.value.destination;

//   // Validate minimum required fields
//   if (!customerCode || !productCode || !freightAmt || !bookDate || !destinationCode) { return; }

//   this.auditService.getChargeValue(apiChargeName, customerCode, productCode, freightAmt, bookDate, destinationCode)
//     .subscribe(
//       (res: any) => {
//         const value = res?.chargeAmount ?? 0;
//         if (this.bookingForm.controls[formControlKey]) {
//           this.bookingForm.controls[formControlKey].patchValue(value);
//         }
//       },
//       (error) => {
//         console.error(`Error fetching charge for ${apiChargeName}`, error);
//       }
//     );
// }
freightCharge() {
  ...
  // });
  //   this.bookingForm.get('ratePerKg')?.valueChanges
  // .pipe(debounceTime(300), distinctUntilChanged())
  // .subscribe(() => {
  //   if (this.rateLockedFromAwb) {
  ...
  renderForm() {
    ...
    this.bookingForm = this.formBuilder.group({
      ...
      volumetricWt: new FormControl(this.volumetricWt, Validators.compose([])),
      chargedWt: new FormControl(this.chargedWt, Validators.compose([])),
      actaulWeightType: new FormControl('', Validators.compose([])),
      ratePerKg: new FormControl(this.ratePerKg, Validators.compose([])),
      freightAmt: new FormControl(this.freightAmt, Validators.compose([Validators.required])),
      ...
      totalAmount: new FormControl(this.totalAmount, Validators.compose([])),
    })
  }
  ...
  openVolumetricModal(consignerCode: any, selectedProduct: any) {
    const dialogRef = this.dialog.open(VolumetricComponent, {
      data: {
        ...
        totalActWeight: this.bookingForm.value.actualWt,
        totalVolWeight: this.bookingForm.value.volumetricWt
      },
      width: '54rem',
      disableClose: true,
      });
...
compareWeights() {
  ...
  // compareWeights() {
  //   const actualWt = this.bookingForm.get('actualWt')?.value;
  //   const volumetricWt = this.bookingForm.get('volumetricWt')?.value ;
  ...
  formSubmit(formData: any) {
    // if (this.bookingForm.valid) {
      ...
      const obj = {
        ...
        CGST: this.CGSTAmt || 0,
        SGST: this.SGSTAmt || 0,
        TotalAmt: formData.totalAmount || 0,
 ...
 this.defaultDate = resp.Data.data.bookDate;
            ...
            this.emailId = resp.Data.data.Consignee_Email;
             this.bookingForm.patchValue({
              ...
              pcs: resp.Data.data.qty,
              containerType: resp.Data.data.box_pcs,
              ratePerKg: resp.Data.data.RatePerkg,
              freightAmt: resp.Data.data.Rate,
              // discountAmt: resp.Data.data.Discount,
              // docketCharge: resp.Data.data.DocketChrgs,
              // fovCharge: resp.Data.data.FOV_Chrgs,
              // odaCharge: resp.Data.data.ODA_Chrgs,
              // packingCharge: resp.Data.data.Charges1,
              // insuranceCharge: resp.Data.data.Charges2,
              // loadingCharge: resp.Data.data.Charges3,
              // otherCharge: resp.Data.data.OtherCharges,
              // fuelCharge: resp.Data.data.FuelCharges,
              // totalAmount: resp.Data.data.TotalAmt,
              // invoiceNo: resp.Data.data.InvoiceNo,
              // invoiceValue: resp.Data.data.InvValue,
              // eWayBill: resp.Data.data.EwayBill,
              // awbNo: resp.Data.data.AwbNo,
              // bookDate: resp.Data.data.bookDate,
              // expectedDeliveryDate: resp.Data.data.dispatchDate
            });
            ...
            // // this.consigneeName = resp.Data.data.Consignee_Name;
            //  this.bookingForm.patchValue({
            //     consigneeName: resp.Data.data.Consignee_Name,
            //     totalAmount: resp.Data.data.TotalAmt,
            //     gstAmount: resp.Data.data.ServiceTax,
            //     ratePerKg: resp.Data.data.RatePerkg,
            //     freightAmt: resp.Data.data.Rate
            // });
            // this.pinCode = resp.Data.data.Consignee_Pin;
  ...
  updateForm() {
    const selectedConsigner = this.consignerList.find(item => item.customerCode === this.bookingForm.value.consignerName);
      ...
      const obj = {
        ...
        CGST: this.CGSTAmt || 0,
        SGST: this.SGSTAmt || 0,
        TotalAmt: this.bookingForm.value.totalAmount || 0,
