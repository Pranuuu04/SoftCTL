...
export class UserLogComponent implements OnInit {
  ...
  'srNo', 'locationName',  'BookDate', 'AwbNo', 'Customer_Name', 'Consignee_Name', 'Mode_Name', 'OriginName',
  'DestinationName', 'ClientType', 'Qty', 'ActualWt', 'ChargedWt', 'UserName', 'Reason', 'ActionDate', 'ActionTime', 'Type'
];
...
downloadSample() {
  ...
  dialogRef.afterClosed().subscribe(result => {
    ...
    const params = {
      ...
      this.AllService.getLogReport(params).subscribe((response: any) => {
  ...
  this.AllService.getLogReport(params).subscribe({
      next: (resp: any) => {
        if (resp.status === 1) {
          this.showTable = true;
      ...
      error: () => {
        this.openSnackBar('Failed to fetch Log reports', 'error-snackbar');
      }
    });
  }
