...
@Component({
  selector: 'app-pending-drs',
  templateUrl: './pending-drs.component.html',
  styleUrls: ['./pending-drs.component.css']
})
export class PendingDRSComponent implements OnInit {
  ...
  getPendingData(pageNumber: number, pageSize: number) {
   if (this.userType !== 'Admin') {
    this.getData.getRunsheetPending( this.sessionLocationCode, pageNumber, pageSize).subscribe((resp: any) => {
      if (resp.status === 1) {
        this.showTable = true;
    ...
    this.getData.getRunsheetPending( this.selectedValue, pageNumber, pageSize).subscribe((resp: any) => {
      if (resp.status === 1) {
        this.showTable = true;
        this.DrsTableData = resp.Data;
        this.dataSource.data = this.DrsTableData;
