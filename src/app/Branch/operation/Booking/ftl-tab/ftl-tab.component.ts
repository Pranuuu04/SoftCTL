...
import { ConsigneeComponent } from 'app/Branch/Shared/consignee/consignee.component';
...
@Component({
  selector: 'app-ftl-tab',
  templateUrl: './ftl-tab.component.html',
  styleUrls: ['./ftl-tab.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,

})
export class FtlTabComponent implements OnInit, AfterViewInit {
  ...
  pinCode: any;
  custType: any;
  billParty: any = 'Consigner';
  containerType: any = 'pcs';
  actaulWeightType: any = 'weight';
  transporterType: string;
  deliveryType: any;
  packageType: any;
  docketAction: any = 'yes';
  fovAction: any = 'yes';
  odaAction: any = 'yes';
  packingAction: any = 'yes';
  insuranceAction: any = 'yes';
  loadingAction: any = 'yes';
  fuelAction: any = 'yes';
  selectedVendor1: any;
  selectedMode: any;
  selectedProduct: any;
  ...
  consigneestateName: any;
  consigneestateCode: any;
  consigneecountryName: any;
  consigneePhoneNo: any;
  consigneeEmail: any;
  consigneeGST: any;
  isEddChecked = true;
...
//  this.consignerCode = localStorage.getItem('consignerCode') || '';
 ...
 fetchViaData(event: any) {
  ...
  ngAfterViewInit() {
    ...
    // this.bookingForm.valueChanges.pipe(debounceTime(500)).subscribe(() => {
    //   this.calculateTotalAmount();
    // });
  ...
  renderForm() {
    ...
    this.bookingForm = this.formBuilder.group({
      ...
      transporterType: new FormControl(this.transporterType, Validators.compose([Validators.required])),
      vehicleType: new FormControl(this.vehicleType, Validators.compose([Validators.required])),
      vehicleNo: new FormControl(this.vehicleNo, Validators.compose([])),
      driverName: new FormControl(this.driverName, Validators.compose([Validators.required])),
      driverNo: new FormControl(this.driverMobile, Validators.compose([])),
      brokerName: new FormControl(this.brockerName, Validators.compose([])),
  ...
  addressList() {
    this.bookingService.getConsigneeDetail(this.selectedConsignee).subscribe(
      (resp) => {
  ...
  openVendorboxModal() {
    const dialogRef = this.dialog.open(VendorboxComponent, {
      data: {
        vendor2: this.selectedVendor2,
        forwardNo2: this.forwardNo2,
        vendor3: this.selectedVendor3,
        forwardNo3: this.forwardNo3
      },
      width: '25rem',
      disableClose: true,
    });
...
getGstDataWithoutModal(subTotalAmt: any, consignerCode: any) {
  ...
  formSubmit(formData: any) {
    // if (this.bookingForm.valid) {
      ...
      if (this.awbType === 'Manual' && (!formData.awbNo || formData.awbNo.trim() === '')) {
      ...
      const obj = {
        ...
        transpoterType : this.transporterType || formData.transporterType || '',
        vehicleType : formData.vehicleType || '',
        driverName : formData.driverName || '',
        driverMobile : formData.driverNo || '',
        vehicleNo : formData.vehicleNo || '',
        brockerName : formData.brokerName || '',
  ...
  findEditAwbNo(event: any) {
    ...
    this.checkBranchSelection();
          ...
          if (resp.status === 1) {
            ...
            this.selectedVendor2 = resp.Data.data.Vendor_Code2;
            this.forwardNo2 = resp.Data.data.VendorAwbNo2;

            this.selectedVendor3 = resp.Data.data.Vendor_Code3;
            this.forwardNo3 = resp.Data.data.VendorAwbNo3;
            ...
            this.transporterType = resp.Data.data.Transpoter_type;
            this.vehicleNo = resp.Data.data.Vehicle_No;
            this.vehicleType = resp.Data.data.Vehicle_Type;
            this.driverMobile = resp.Data.data.Driver_Mobile;
            this.driverName = resp.Data.data.Driver_Name;
            this.brockerName = resp.Data.data.Brocker_Name;
  ...
  updateForm() {
    const selectedConsigner = this.consignerList.find(item => item.customerCode === this.bookingForm.value.consignerName);
      ...
      const obj = {
        ...
        transpoterType : this.transporterType || this.bookingForm.value.transporterType || '',
        vehicleType : this.bookingForm.value.vehicleType || '',
        driverName : this.bookingForm.value.driverName || '',
        driverMobile : this.bookingForm.value.driverNo || '',
        vehicleNo : this.bookingForm.value.vehicleNo || '',
        brockerName : this.bookingForm.value.brokerName || '',
        ...
        ShipperEmail: this.shipperEmail || '',
        ShippergstNo: this.shipperGSTNo || '',
        ShipperName: this.shipperName || this.selectedShipper || '',
        Shipperphone: this.shipperPhone || '',
        Shipperpin: this.shipperPinCode || '',
        Shipperstatecode: this.shipperStateCode || '',
        // DestinationCode: this.DestinationCode || '',
  ...
  onVehicleTypeChange(event: any) {
    ...
    if (this.selectedType === 'Self') {
      this.httpService.get(`${environment.apiUrl}Manifest/getVehicleNo`).then(resp => {
        this.vehicleNumbers = resp.Data;
      })
    } else {
    }
}
...
loadDriverName() {
  this.AllService.getDriver().subscribe((resp: any) => {
...
onDriverNameChange(event: any) {
  ...
  if (selectedDriverCode != '') {
    this.AllService.getDriverMobile(selectedDriverCode).subscribe((response: any) => {
        if (response.status === 1) {
          this.driverNumber = response.Data[0].driverMobileNo;
          console.log('this.driverNumber' , this.driverNumber);
