...
formSubmit(reason: string): void {
  ...
  if (this.awbDisabled) {
    ...
    this.http.get(`${environment.apiUrl}runsheet/deleteRunsheet?sessionLocationCode=${this.Location}&awbNo=${obj.awbNo}&Reason=${reason}&userName=${this.username}`).then(resp => {
      if (resp.status === 1) {
    ...
    this.http.get(`${environment.apiUrl}runsheet/deleteRunsheet?sessionLocationCode=${this.sessionLocationCode}&drsNo=${obj.DrsNo}&Reason=${reason}&userName=${this.username}`).then(resp => {
      if (resp.status === 1) {
        this.openSnackBar( resp.message, 'custom-snackbar');
        this.CloseDialog();
      } else {
